...
local function addEnemyAndAllySettings(self, mainFrame)
	...
	-- Local table definitions for option validation scoping
	local Buttons = {
		Target = TARGET,
		Focus = SET_FOCUS,
		...
		settings[BattleGroundEnemies:GetPlayerCountConfigName(playerCountLocation)] = {
			...
			args = {
				...
				LoadFromDefaultPlayerCountProfile = {
					...
					values = function()
						local t = {}
						for k = 1, #playerCountConfigDefaults do
							table.insert(t, L[playerType]..": ".. BattleGroundEnemies:GetPlayerCountConfigNameLocalized(playerCountConfigDefaults[k], false))
						end
						return t
					end,
					confirm = function ()
...
function BattleGroundEnemies:SetupOptions()
	...
	-- Cache Data tables locally for options to avoid nil references during validation
	local Buttons = {
		Target = TARGET,
		Focus = SET_FOCUS,
		Custom = L.UserDefined
