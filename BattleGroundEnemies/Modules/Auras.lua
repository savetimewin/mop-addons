...
local function createNewAuraFrame(playerButton, container)
	local childFrame = CreateFrame('Button', nil, container)
	
	-- Create icon texture (replace CompactAuraTemplate functionality)
	childFrame.icon = childFrame:CreateTexture(nil, "BACKGROUND")
	childFrame.icon:SetAllPoints()
	
	-- Create cooldown object
	childFrame.cooldown = CreateFrame("Cooldown", nil, childFrame, "CooldownFrameTemplate")
	childFrame.cooldown:SetAllPoints()
	
	-- Create count FontString
	childFrame.count = childFrame:CreateFontString(nil, "OVERLAY", "NumberFontNormal")
...
local function setupAuraFrame(container, auraFrame, auraDetails)
	...
	if container.filter == "HELPFUL" then
		...
		if DebuffTypeColor then
			...
			-- Fallback for Classic where DebuffTypeColor may not exist
			color = {r = 1, g = 1, b = 1}
		end
